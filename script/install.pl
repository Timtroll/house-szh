...
# миграция: чтение и запуск sql файлов из house/sql
...
#                                     test  - только база данных house_test
# необязательные опции:
# --start=test  - по окончании работы скрипта, старт mojo с указанной базой, если не указано, то стартует с базой house
...
my ( $self, $path_sql, $path_conf, $path_log, $check_house, $config_update, $base );
...
helpme('need_mode') unless grep( /^$mode$/, ( 'all', 'test', 'house' ) );

$options{'rebuild'} = ( exists $options{'rebuild'} && $options{'rebuild'} ) ? 1 : 0;

$mode = $options{'start'};
$options{'start'} = ( exists $options{'start'} && $options{'start'} && grep( /^$mode$/, ( 'test', 'house' ) ) ) ? $options{'start'} : 'house';
...
my $name = ( $options{'mode'} eq 'test' ) ? 'house_test' : 'house';
# $name = 'house_test' if $options{'mode'} eq 'test';
# $name = 'house' if $options{'mode'} eq 'house';
...
sub all_one_test {
    ...
    # проверяем наличие баз house,house_test удаляем и создаем заново, если нужно
    # ( rebuild=1 ) -  персоздаем базу

    if ( $options{'rebuild'} ) {
        $base = ('house') if $options{'mode'} eq 'house';
        $base = ('house_test') if $options{'mode'} eq 'test';

        if ( check_db( $self, $base ) ) {
            # удаляем старую базу house
            delete_db( $self, $base );
        }
    ...
    if ( $options{'rebuild'} ) {
        my $connect = ( $base =~ /test/ ) ? $config_update->{'databases'}->{'pg_main_test'} : $config_update->{'databases'}->{'pg_main'};
        $self->{dbh} = connect_db( $connect );
        $config_update->{'test'} = $base eq 'house' ? 0 : 1;

        # стартуем mojo с базой house_test
        write_config( $config_update );
